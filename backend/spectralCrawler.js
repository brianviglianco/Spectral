...
async function runCrawl(url){
  ...
  try { console.log((await clickReject(pageA)) ? 'Clicked OneTrust Reject' : 'OneTrust Reject not found'); } catch { console.log('OneTrust Reject click error'); }
  await sleep(400);
  ...
  const cmpInitialB = await waitForCMPOverlay(pageB, { timeoutMs:5000, retry:3 });
  const acceptPre  = await captureStage(pageB, 'accept_pre',  siteETLD);
  try { console.log((await clickAccept(pageB)) ? 'Clicked OneTrust Accept' : 'OneTrust Accept not found'); } catch { console.log('OneTrust Accept click error'); }
  await sleep(500);
  ...
  for (const c of [cmpInitial, cmpInitialB, cmpFinalA, cmpFinalB]) {
    if (c && c.detected && c.provider && c.provider !== 'None') providersSet.add(c.provider);
  }

  const bannerSummary = {
    initial: cmpInitial,
    initialB: cmpInitialB,
    finalA: cmpFinalA,
    finalB: cmpFinalB,
    anyDetected: providersSet.size > 0,
    providers: Array.from(providersSet),
    buttons: {
      reject:  !!(cmpInitial?.reject  || cmpFinalA?.reject  || cmpFinalB?.reject  || cmpInitialB?.reject),
      accept:  !!(cmpInitial?.accept  || cmpFinalA?.accept  || cmpFinalB?.accept  || cmpInitialB?.accept),
      settings:!!(cmpInitial?.settings|| cmpFinalA?.settings|| cmpFinalB?.settings|| cmpInitialB?.settings)
    }
  };
