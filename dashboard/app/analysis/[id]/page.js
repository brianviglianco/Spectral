    ...
    <div className="min-h-screen flex flex-col bg-black">
      <nav className="bg-black/80 backdrop-blur-xl border-b border-white/10">
        <div className="max-w-[1600px] mx-auto px-8 py-4 flex justify-between items-center">
          ...
          <Link href="/" className="px-5 py-2.5 bg-white/5 text-gray-400 border border-white/10 rounded-lg text-sm font-semibold hover:bg-white/10 transition">
            Back to List
          </Link>
        </div>
      </nav>
      ...
      <div className="flex flex-1 max-w-[1600px] mx-auto w-full">
        <aside className="w-72 bg-white/5 border-r border-white/10 p-8">
          ...
          <nav className="space-y-1">
            <button onClick={() => setActiveTab('overview')} className={`w-full flex items-center gap-3 px-4 py-3 text-sm rounded-lg transition ${activeTab === 'overview' ? 'text-purple-400 bg-purple-500/10' : 'text-gray-400 hover:bg-white/5'}`}>
              <span>📊</span> Overview
            </button>
            <button onClick={() => setActiveTab('violations')} className={`w-full flex items-center gap-3 px-4 py-3 text-sm rounded-lg transition ${activeTab === 'violations' ? 'text-purple-400 bg-purple-500/10' : 'text-gray-400 hover:bg-white/5'}`}>
              <span>⚠️</span> Violations
            </button>
            {stages && (
              <button onClick={() => setActiveTab('stages')} className={`w-full flex items-center gap-3 px-4 py-3 text-sm rounded-lg transition ${activeTab === 'stages' ? 'text-purple-400 bg-purple-500/10' : 'text-gray-400 hover:bg-white/5'}`}>
                <span>📈</span> Stages
              </button>
            )}
            <button onClick={() => setActiveTab('reports')} className={`w-full flex items-center gap-3 px-4 py-3 text-sm rounded-lg transition ${activeTab === 'reports' ? 'text-purple-400 bg-purple-500/10' : 'text-gray-400 hover:bg-white/5'}`}>
              <span>📄</span> Reports
            </button>
            <button onClick={() => setActiveTab('download')} className={`w-full flex items-center gap-3 px-4 py-3 text-sm rounded-lg transition ${activeTab === 'download' ? 'text-purple-400 bg-purple-500/10' : 'text-gray-400 hover:bg-white/5'}`}>
              <span>📦</span> Download
            </button>
          </nav>
        </aside>
        ...
        <main className="flex-1 p-8 overflow-y-auto">
          <div className="mb-8">
            ...
            <div className="flex gap-3">
              <span className={`px-4 py-2 rounded-lg text-sm font-semibold ${report.analysisMode === 'full' ? 'bg-purple-500/15 text-purple-400 border border-purple-500/30' : 'bg-purple-500/15 text-purple-400 border border-purple-500/30'}`}>
                {report.analysisMode?.toUpperCase()}
              </span>
              <span className="px-4 py-2 bg-white/5 text-gray-400 border border-white/10 rounded-lg text-sm font-semibold">
          ...
          {activeTab === 'overview' && (
            <div className="space-y-6">
                      ...
                      Object.entries(dimensions).map(([key, score]) => (
                        <div key={key} className="mb-6">
                          <div className="flex justify-between items-center mb-2">
                            <span className="text-sm text-gray-400 capitalize font-medium">{key.replace(/_/g, ' ')}</span>
                            <span className="text-sm font-bold text-white">{score}%</span>
                          </div>
                          <div className="h-3 bg-white/5 rounded-full overflow-hidden">
                            <div className="h-full bg-gradient-to-r from-purple-500 to-purple-600 transition-all" style={{ width: `${score}%` }} />
                          </div>
                        </div>
                      ))
                    ) : (
                      <p className="text-gray-400">No dimension data available</p>
                    )}
                  </div>
                </div>
              </section>
              ...
              <section className="grid grid-cols-4 gap-4">
                ...
                <div className="bg-white/5 border border-white/10 rounded-xl p-6">
                  <div className="text-gray-400 text-sm mb-2">Total Cookies</div>
                  <div className="text-4xl font-bold text-white">{totalCookies}</div>
                </div>
                <div className="bg-white/5 border border-white/10 rounded-xl p-6">
                  <div className="text-gray-400 text-sm mb-2">Tracking Hits</div>
              ...
              <section className="bg-white/5 border border-white/10 rounded-xl p-8">
                ...
                <div className="grid grid-cols-4 gap-4">
                  ...
                  <div className="text-center p-6 bg-orange-500/10 border border-orange-500/30 rounded-lg">
                    ...
                    <div className="text-xs text-gray-500 mt-1">Priority fixes needed</div>
                  </div>
                  <div className="text-center p-6 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
                    ...
                    <div className="text-xs text-gray-500 mt-1">Should be addressed</div>
                  </div>
                  <div className="text-center p-6 bg-gray-700/10 border border-gray-700/30 rounded-lg">
                    <div className="text-5xl font-extrabold text-gray-400 mb-2">{violationsBySeverity.low}</div>
                    <div className="text-sm font-semibold text-gray-400 uppercase tracking-wider">Low</div>
                    <div className="text-xs text-gray-500 mt-1">Minor improvements</div>
                  </div>
                </div>
              </section>
              ...
              <section className="bg-white/5 border border-white/10 rounded-xl p-8">
                ...
                <div className="grid grid-cols-2 gap-6">
                  ...
                  <div>
                    <div className="text-sm text-gray-400 mb-2">Implementation</div>
                  ...
                  {report.cmp?.version && (
                    <div>
                      ...
                      <div className="text-xl font-mono text-purple-400">{report.cmp.version}</div>
                    </div>
                  )}
                  {preConsentViolations > 0 && (
                    <div>
                      <div className="text-sm text-gray-400 mb-2">Pre-consent Issues</div>
                    ...
                    {stages.map((stage, idx) => {
                      ...
                      return (
                        <div key={idx} className={`p-4 rounded-lg text-center border ${status.bgColor}`}>
                          <div className="text-xs text-gray-400 uppercase mb-2">{stage.stage}</div>
                          <div className={`text-2xl font-bold ${status.color}`}>
                            {stage.trackingHits}
                          </div>
                          <div className="text-xs text-gray-500 mt-1">hits</div>
              ...
              <div className="bg-white/5 border border-white/10 rounded-xl p-8">
                ...
                <div className="space-y-4">
                  {violations.map((v, i) => {
                    ...
                    return (
                      <div key={i} className={`bg-white/5 border rounded-lg overflow-hidden transition-all ${v.severity === 'CRITICAL' ? 'border-red-500/50' : v.severity === 'HIGH' ? 'border-orange-500/50' : v.severity === 'MEDIUM' ? 'border-yellow-500/50' : 'border-gray-500/50'}`}>
                        <div 
                          className="p-6 cursor-pointer hover:bg-white/10 transition"
                          onClick={() => setExpandedViolation(isExpanded ? null : i)}
                        >
                          <div className="flex items-start justify-between mb-3">
                            <div className="flex-1">
                              <div className="flex items-center gap-3 mb-2">
                                <span className="font-mono text-sm font-bold text-purple-400">{v.code}</span>
                                <span className={`px-3 py-1 rounded text-xs font-semibold ${v.severity === 'CRITICAL' ? 'bg-red-500/15 text-red-400' : v.severity === 'HIGH' ? 'bg-orange-500/15 text-orange-400' : v.severity === 'MEDIUM' ? 'bg-yellow-500/15 text-yellow-400' : 'bg-gray-500/15 text-gray-400'}`}>
                                  {v.severity}
                                </span>
                              </div>
                            ...
                            <button className="text-gray-400 hover:text-purple-400 transition ml-4">
                              {isExpanded ? '▼' : '▶'}
                            </button>
                          </div>
                        </div>
                            ...
                            <div className="p-6 space-y-6">
                              ...
                              {v.evidence && (
                                <div>
                                  ...
                                  <div className="bg-black/50 rounded-lg p-4 font-mono text-xs text-gray-400 overflow-x-auto">
                  ...
                  {stages.map((stage, idx) => {
                    ...
                    return (
                      <div key={idx} className={`border rounded-lg overflow-hidden ${status.bgColor}`}>
                          ...
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-6">
                              <div className="text-center">
                                ...
                                <div className="text-xs text-gray-500 uppercase">{stage.stage}</div>
                              </div>
                              ...
                              <div className="flex gap-8">
                                ...
                                <div>
                                  ...
                                  <div className="text-2xl font-bold text-purple-400">{thirdPartyScriptsCount}</div>
                                </div>
                                <div>
                                ...
                                <div>
                                  <div className="text-sm text-gray-400">Tracking Hits</div>
                        ...
                        {isExpanded && (
                          <div className="border-t border-white/10 bg-black/60 p-6 space-y-6">
                                ...
                                <div className="space-y-2">
                                  {stageViolations.map((v, i) => (
                                    <div key={i} className="bg-red-500/10 border border-red-500/30 rounded-lg p-3">
                                      ...
                                      <div className="text-xs text-gray-300 mt-1">{v.description}</div>
                                    </div>
                                  ))}
                                </div>
                              </div>
                            ...
                            <div>
                              ...
                              <div className="grid grid-cols-3 gap-3">
                                <div className="bg-white/5 rounded-lg p-4 text-center">
                                  ...
                                  <div className="text-xs text-gray-400 mt-1">Tracking</div>
                                </div>
                                <div className="bg-white/5 rounded-lg p-4 text-center">
                                  <div className="text-2xl font-bold text-green-400">{cookieBreakdown.consent || 0}</div>
                                  <div className="text-xs text-gray-400 mt-1">Consent</div>
                                </div>
                                <div className="bg-white/5 rounded-lg p-4 text-center">
                                  ...
                                  <div className="space-y-2">
                                    {thirdPartyScriptsOnly.slice(0, 10).map((script, i) => (
                                      <div key={i} className="flex items-center gap-2 text-xs">
                                        ...
                                        <span className="text-gray-300 font-mono truncate text-[10px]">
                                          {script.src || 'inline script'}
                                        </span>
                                      </div>
                                    ))}
                                    {thirdPartyScriptsOnly.length > 10 && (
                                      <div className="text-xs text-gray-500 italic">+ {thirdPartyScriptsOnly.length - 10} more scripts...</div>
                                    )}
                                  </div>
                                </div>
                              </div>
                                  ...
                                  <div className="space-y-2">
                                    {networkRequests.slice(0, 8).map((req, i) => (
                                      <div key={i} className="flex items-start gap-2 text-xs">
                                        <span className={`${req.resourceType === 'xhr' ? 'text-purple-400' : req.resourceType === 'script' ? 'text-purple-400' : 'text-gray-500'}`}>
                                          {req.method || 'GET'}
                                        </span>
                                        <span className="text-gray-400 font-mono text-[10px] truncate flex-1">{req.url}</span>
                                      </div>
                                    ))}
                                    {networkRequests.length > 8 && (
                                      <div className="text-xs text-gray-500 italic">+ {networkRequests.length - 8} more requests...</div>
                                    )}
                                  </div>
                                </div>
                              </div>
                              ...
                              <div>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                  <div className="bg-white/5 rounded-lg p-4">
                                    <div className="text-sm text-gray-400 mb-2">LocalStorage: {localStorageKeys.length} keys</div>
                                    {localStorageKeys.length > 0 && (
                                      <div className="space-y-1 max-h-24 overflow-y-auto">
                                        {localStorageKeys.slice(0, 5).map((key, i) => (
                                          <div key={i} className="text-xs text-gray-500 font-mono truncate">{key}</div>
                                        ))}
                                        {localStorageKeys.length > 5 && (
                                          <div className="text-xs text-gray-600 italic">+{localStorageKeys.length - 5} more...</div>
                                        )}
                                      </div>
                                    )}
                                  </div>
                                  <div className="bg-white/5 rounded-lg p-4">
                                    <div className="text-sm text-gray-400 mb-2">SessionStorage: {sessionStorageKeys.length} keys</div>
                                    {sessionStorageKeys.length > 0 && (
                                      <div className="space-y-1 max-h-24 overflow-y-auto">
                                        ...
                                        {sessionStorageKeys.length > 5 && (
                                          <div className="text-xs text-gray-600 italic">+{sessionStorageKeys.length - 5} more...</div>
                                        )}
                                      </div>
                                    )}
                                  </div>
                            ...
                            <div>
                              ...
                              <div className="grid grid-cols-2 gap-4">
                                <div className="bg-white/5 rounded-lg p-4">
                                  <div className="text-sm text-gray-400 mb-1">Banner Visibility</div>
                                  <div className={`text-lg font-semibold ${bannerState === 'visible' ? 'text-purple-400' : bannerState === 'hidden' ? 'text-gray-500' : 'text-gray-600'}`}>
                                    {bannerState === 'visible' ? '👁️ Visible' : bannerState === 'hidden' ? '🚫 Hidden' : '❓ Unknown'}
                                  </div>
                                </div>
                                {cmpState && (
                                  <div className="bg-white/5 rounded-lg p-4">
                                    <div className="text-sm text-gray-400 mb-1">CMP Data</div>
                                    <div className="text-xs font-mono text-purple-400 truncate max-w-full overflow-hidden">
          ...
          {activeTab === 'reports' && (
            <section className="bg-white/5 border border-white/10 rounded-xl p-8">
              ...
              <div className="flex gap-2 mb-6">
                <button onClick={() => setActiveReport('console')} className={`px-4 py-2 text-sm font-semibold rounded-lg transition ${activeReport === 'console' ? 'bg-purple-500/10 text-purple-400 border border-purple-500' : 'bg-white/5 text-gray-400 hover:bg-white/10'}`}>💻 Console</button>
                <button onClick={() => setActiveReport('marketing')} className={`px-4 py-2 text-sm font-semibold rounded-lg transition ${activeReport === 'marketing' ? 'bg-purple-500/10 text-purple-400 border border-purple-500' : 'bg-white/5 text-gray-400 hover:bg-white/10'}`}>📈 Marketing</button>
                <button onClick={() => setActiveReport('technical')} className={`px-4 py-2 text-sm font-semibold rounded-lg transition ${activeReport === 'technical' ? 'bg-purple-500/10 text-purple-400 border border-purple-500' : 'bg-white/5 text-gray-400 hover:bg-white/10'}`}>🔧 Technical</button>
                <button onClick={() => setActiveReport('legal')} className={`px-4 py-2 text-sm font-semibold rounded-lg transition ${activeReport === 'legal' ? 'bg-purple-500/10 text-purple-400 border border-purple-500' : 'bg-white/5 text-gray-400 hover:bg-white/10'}`}>⚖️ Legal</button>
              </div>
              <div className="bg-black/50 rounded-lg p-6 max-h-[600px] overflow-y-auto">
                <pre className="text-sm text-gray-300 whitespace-pre-wrap font-mono leading-relaxed">{reportFiles[activeReport] || 'Report not available'}</pre>
              </div>
            </section>
          )}
          ...
          {activeTab === 'download' && (
            <section className="bg-white/5 border border-white/10 rounded-xl p-8">
              ...
              <a href={`/api/download?id=${params.id}`} className="block">
                <div className="bg-gradient-to-br from-purple-500/15 to-purple-600/15 border-2 border-purple-500 rounded-xl p-8 hover:from-purple-500/25 hover:to-purple-600/25 transition-all hover:shadow-2xl hover:shadow-purple-500/20">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="grid grid-cols-2 gap-4 mb-6">
                        <div className="bg-black/50 rounded-lg p-4 border border-white/10">
                          <div className="text-purple-400 font-semibold mb-2">📄 Reports Included</div>
                        ...
                        <div className="bg-black/50 rounded-lg p-4 border border-white/10">
                          <div className="text-purple-400 font-semibold mb-2">🔍 Evidence Files</div>
                          <ul className="text-sm text-gray-400 space-y-1">
